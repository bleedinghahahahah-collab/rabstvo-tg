...
// ---- Daily "peak online" report: sent once a day to ADMIN_ID only (not
// broadcast to players). Tracked in-memory and checked on the same 3s tick
// as everything else below, so the report goes out within a few seconds of
// the calendar day rolling over on the server's local clock. ----
let dailyPeakOnline = 0;
let dailyPeakDate = new Date().toDateString();
...
function refreshOnlineCount() {
  ...
  if (cachedOnlineCount > dailyPeakOnline) dailyPeakOnline = cachedOnlineCount;
}
refreshRankCache();
refreshOnlineCount();

function checkDailyOnlinePeakRollover() {
  const today = new Date().toDateString();
  if (today === dailyPeakDate) return;
  const peak = dailyPeakOnline;
  const forDate = dailyPeakDate;
  dailyPeakDate = today;
  dailyPeakOnline = cachedOnlineCount; // seed the new day instead of starting from 0
  for (const adminId of getAdminIds()) {
    bot.api.sendMessage(adminId, `📊 Максимальный онлайн за ${forDate}: ${peak} игроков одновременно`).catch(() => {});
  }
}
...
function buildLiveSnapshot() {
  const byBalance = topByBalance(LEADERBOARD_LIMIT).map((u, i) => ({
    rank: i + 1,
    id: u.id,
    username: u.username,
  ...
  const byOwned = topByOwned(ownedCount, LEADERBOARD_LIMIT).map((u, i) => ({
    rank: i + 1,
    id: u.id,
    username: u.username,
    first_name: u.first_name,
...
app.post('/api/casino/bet', requireAuth, (req, res) => {
  ...
  const result = casino.placeBet(
    req.userId,
    req.body.amount,
    { username: me.username, first_name: me.first_name },
    req.body.autoCashoutAt
  );
  ...
  res.json(
    rows.map((u) => {
      ...
      return {
        ...
        whip_available_at: u.whip_cooldown_until || 0,
      };
    })
  );
});

// ---- POST /api/whip/:id — purely social: notifies the owned person, no
// economic effect. 5-minute cooldown per person, enforced server-side. ----
app.post('/api/whip/:id', requireAuth, (req, res) => {
  const targetId = Number(req.params.id);
  const result = tryWhip(req.userId, targetId);
  if (!result.ok) {
    if (result.error === 'cooldown') {
      return res.status(429).json({ error: 'Подожди, пока пройдёт перезарядка плети', unlock_at: result.unlock_at });
    }
    return res.status(400).json({ error: 'Это не твой человек' });
  }
  notify(targetId, 'Твой господин отшлепал тебя плетью. 🔗');
  res.json({ ok: true, unlock_at: result.unlock_at });
});
...
app.get('/api/leaderboard', requireAuth, (req, res) => {
  ...
  const top = by === 'owned' ? topByOwned(ownedCount, LEADERBOARD_LIMIT) : topByBalance(LEADERBOARD_LIMIT);
  res.json(
    top.map((u, i) => ({
      rank: i + 1,
      id: u.id,
...
// ---- GET /api/shield/coin-status — is the coin-bought shield active, what does it cost ----
app.get('/api/shield/coin-status', requireAuth, (req, res) => {
  res.json(coinShieldStatus(getUser(req.userId)));
});

// ---- POST /api/shield/coin-buy — buy 1h of shield with in-game coins
// (the "free" shop) instead of Telegram Stars. ----
app.post('/api/shield/coin-buy', requireAuth, (req, res) => {
  const result = tryBuyCoinShield(req.userId);
  if (!result.ok) {
    if (result.error === 'already_active') {
      return res.status(400).json({ error: 'Защита уже активна', shield_until: result.shield_until });
    }
    return res.status(400).json({ error: 'Недостаточно монет', cost: result.cost });
  }
  res.json(result);
